...
class EmbeddingBasedMethod(LFramework):
    def __init__(self, args, kg, mdl, secondary_kg=None, tertiary_kg=None, logger=None):
        super(EmbeddingBasedMethod, self).__init__(args, kg)
    ...
    def learn(
        self,
        mini_batch: Tuple[torch.Tensor, torch.Tensor, torch.Tensor]
    ):
        ...
        return {'print_loss': float(loss.item())}

    def loss(self, mini_batch):
        kg, mdl = self.kg, self.mdl
        # compute object training loss
        e1, r, e2 = mini_batch #format_batch(mini_batch, num_labels=kg.num_entities, device=self.device)
    ...
    def predict(self, mini_batch):
        self.eval()
        kg, mdl = self.kg, self.mdl
        e1, r, e2 = mini_batch # format_batch(mini_batch, device=self.device)
        with torch.no_grad():
            pred_scores = mdl.forward(e1, r, kg)
        return pred_scores
    ...
    def get_subject_mask(self, e1_space, e2, q):
        ...
        for i in range(len(e1_space)):
            ...
            if not _e2 in answer_vectors or not _q in answer_vectors[_e2]:
                answer_vector = torch.LongTensor([[kg.num_entities]])
            else:
    ...
    #                 e1_id, r_id, e2_id = kg.triple2ids((e1, r, e2))
    ...
    #
    #     o_f = open(os.path.join(self.data_dir, 'train.fuzzy.txt'), 'w')
    #     print('Saving fuzzy facts to {}'.format(os.path.join(self.data_dir, 'train.fuzzy.txt')))
    #     count = 0
    ...
    #         e1 = torch.LongTensor(e1_ids_b)
    #         r = torch.LongTensor(r_ids_b)
    #         pred_scores = mdl.forward(e1, r, kg)
    ...
    #             e2_ids.append(e2_id)
    #             r_ids.append(r_id)
    #     e1 = torch.arange(kg.num_entities)
    #     for i in range(len(e2_ids)):
    #         r = torch.full(e1.size(), r_ids[i])
    #         e2 = torch.full(e1.size(), e2_ids[i])
