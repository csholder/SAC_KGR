...
class KnowledgeGraph(nn.Module):
    ...
    def __init__(self, args):
        ...
        print('** Create {} knowledge graph **'.format(args.model_name))
    ...
    def load_graph_data(self, data_dir):
        ...
        if self.args.model_name.startswith('rl'):
    ...
    def vectorize_action_space(self, data_dir):
        ...
        def vectorize_action_space(action_space_list, action_space_size):
            ...
            r_space = torch.zeros((bucket_size, action_space_size), dtype=torch.long, device=self.device) + self.dummy_r
            e_space = torch.zeros((bucket_size, action_space_size), dtype=torch.long, device=self.device) + self.dummy_e
            action_mask = torch.zeros((bucket_size, action_space_size), device=self.device)
        ...
        def vectorize_unique_r_space(unique_r_space_list, unique_r_space_size, volatile):
            ...
            unique_r_space = torch.zeros((bucket_size, unique_r_space_size), dtype=torch.long, device=self.device) + self.dummy_r
            for i, u_r_s in enumerate(unique_r_space_list):
                for j, r in enumerate(u_r_s):
                    unique_r_space[i, j] = r
            return torch.tensor(unique_r_space, dtype=torch.long, device=self.device)
        ...
        if self.args.use_action_space_bucketing:
            ...
            num_facts_saved_in_action_table, max_key = 0, -1
            for e1 in range(self.num_entities):
                ...
                max_key = max_key if key <= max_key else key
            ...
            self.max_num_actions = max_key * self.args.bucket_interval
        else:
            action_space_list = []
            max_num_actions = 0
            for e1 in range(self.num_entities):
            ...
            self.action_space = vectorize_action_space(action_space_list,
                                                       max_num_actions)  # r_space, e_space, action_mask
            self.max_num_actions = max_num_actions

    def load_all_answers(self, data_dir, add_reversed_edges=False):
        def add_subject(e1, r, e2, d):
            if not e2 in d:
                d[e2] = {}
            if not r in d[e2]:
                d[e2][r] = set()
        ...
        add_subject(self.dummy_e, self.dummy_r, self.dummy_e, train_subjects)
        add_subject(self.dummy_e, self.dummy_r, self.dummy_e, dev_subjects)
        add_subject(self.dummy_e, self.dummy_r, self.dummy_e, all_subjects)
        add_object(self.dummy_e, self.dummy_r, self.dummy_e, train_objects)
        add_object(self.dummy_e, self.dummy_r, self.dummy_e, dev_objects)
        add_object(self.dummy_e, self.dummy_r, self.dummy_e, all_objects)
        for file_name in ['raw.kb', 'train.txt', 'dev.txt', 'test.txt']:
            with open(os.path.join(data_dir, file_name)) as f:
                for line in f:
                    e1, r, e2 = line.strip().split()
                    e1, r, e2 = self.triple2ids((e1, r, e2))
                    if file_name in ['raw.kb', 'train.txt']:
                        add_subject(e1, r, e2, train_subjects)
                        add_object(e1, r, e2, train_objects)
                        if add_reversed_edges:
                            add_subject(e2, self.get_inv_relation_id(r), e1, train_subjects)
                            add_object(e2, self.get_inv_relation_id(r), e1, train_objects)
                    if file_name in ['raw.kb', 'train.txt', 'dev.txt']:
                        add_subject(e1, r, e2, dev_subjects)
                        add_object(e1, r, e2, dev_objects)
                        if add_reversed_edges:
                            add_subject(e2, self.get_inv_relation_id(r), e1, dev_subjects)
                            add_object(e2, self.get_inv_relation_id(r), e1, dev_objects)
                    add_subject(e1, r, e2, all_subjects)
                    add_object(e1, r, e2, all_objects)
                    if add_reversed_edges:
                        add_subject(e2, self.get_inv_relation_id(r), e1, all_subjects)
                        add_object(e2, self.get_inv_relation_id(r), e1, all_objects)
        ...
        def answers_to_var(d_l):
            ...
            for x in d_l:
                for y in d_l[x]:
                    d_v[x][y] = torch.LongTensor(list(d_l[x][y])).unsqueeze(1).to(self.device)
            return d_v
    ...
    def load_fuzzy_facts(self):
        ...
        dev_path = os.path.join(self.args.data_dir, 'dev.txt')
        ...
        theta = 0.5
        fuzzy_fact_path = os.path.join(self.args.data_dir, 'train.fuzzy.txt')
        count = 0
        with open(fuzzy_fact_path) as f:
            for line in f:
                e1, r, e2, score = line.strip().split()
                score = float(score)
                if score < theta:
                    continue
                print(line)
                if '{}\t{}\t{}'.format(e1, r, e2) in removed_triples:
                    continue
    ...
    def get_inv_relation_id(self, r_id):
        if r_id == NO_OP_RELATION_ID:
            return r_id
        if (r_id - NO_OP_RELATION_ID) % 2 == 1:
            return r_id + 1
        else:
            return r_id - 1

    def get_inv_relation_id_batch(self, r_id_batch):
        r_id_batch_1, r_id_batch_2 = r_id_batch + 1, r_id_batch - 1
        no_op_mask = (r_id_batch == NO_OP_RELATION_ID)
        no_op_tensor = torch.ones_like(r_id_batch) * NO_OP_RELATION_ID
        r_id_batch_3 = torch.where((r_id_batch - NO_OP_RELATION_ID) % 2 == 1, r_id_batch_1, r_id_batch_2)
        return torch.where(no_op_mask, no_op_tensor, r_id_batch_3)
    ...
    def virtual_step(self, e_set, r):
        ...
        for i in range(len(e_space)):
            e_set_out_b = torch.tensor(torch.unique(e_space[i].data), device=self.device)
            e_set_out.append(e_set_out_b.unsqueeze(0))
        e_set_out = pad_and_cat(e_set_out, padding_value=self.dummy_e)
        return e_set_out

    def id2triples(self, triple):
        e1, r, e2 = triple
        return self.id2entity[e1], self.id2relation[r], self.id2entity[e2]

    def triple2ids(self, triple):
        e1, r, e2 = triple
        return self.entity2id[e1], self.relation2id[r], self.entity2id[e2]

    def define_modules(self):
        # if not self.args.relation_only:
        self.entity_embeddings = nn.Embedding(self.num_entities, self.entity_dim)
        if self.args.model_name == 'embed.complex':
