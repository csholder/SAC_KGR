...
class DuelDQN(OffPolicyAlgorithm):
    ...
    def __init__(
        ...
        if isinstance(net_arch, str):
            net_arch = list(map(lambda x: int(x), net_arch.strip().split('_')))
    ...
    def _setup_model(self) -> None:
        self.policy_kwargs.update({
            ...
            'action_dropout_final_rate': self.action_dropout_final_rate,
            'action_dropout_fraction': self.action_dropout_fraction,
    ...
    def beam_search_q_value(
        ...
        for t in range(self.num_rollout_steps):
            action_space, q_values = self.policy.calculate_q_values(self._last_obs, self.kg,
    ...
    def beam_search_probability(
        ...
        self._last_obs = start_obs
        for t in range(self.num_rollout_steps):
            action_space, action_dist = self.policy.calculate_action_dist(self._last_obs, self.kg,
                                                                          self.use_action_space_bucketing,
    ...
    def update_progress_state(self, epoch_id):
        ...
        self.policy._update_action_dropout(self._current_progress_remaining)
        ...
        self.logger.info("train/action_dropout_rate:   {}".format(self.policy.action_dropout_rate))
