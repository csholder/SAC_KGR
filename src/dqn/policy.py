        ...
        if use_action_space_bucketing:
            ...
            for action_space_b, reference_b, obs_b in zip(db_action_spaces, db_references, db_observations):
                ...
                if not self.boltzmann_exploration:
                    ...
                    if apply_action_dropout:
                        action_dist_b = apply_dist_action_dropout_mask(action_dist_b, action_mask_b)
                    # Boltzmann Exploration
                    action_indice_b = th.multinomial(action_dist_b, num_samples=1)
                    q_value_b = utils.batch_lookup(q_values_b, action_indice_b)
