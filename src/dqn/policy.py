...
class QNetwork(BaseCritic):
    ...
    def predict_q_value_batch(self, observation: Observation, action_space, kg) -> th.Tensor:
        def apply_action_dropout_mask(q_values, action_mask):
        ...
        q_values = apply_action_dropout_mask(q_values, action_mask)
        return q_values
    ...
    def sample_action(self, obs: Observation, kg, use_action_space_bucketing=False):
        if use_action_space_bucketing:
            ...
            for action_space_b, reference_b, obs_b in zip(db_action_spaces, db_references, db_observations):
                q_values_b = self.q_net.predict_q_value_batch(obs_b, action_space_b, kg)
                # Greedy action
            ...
            q_values = self.q_net.predict_q_value_batch(obs, action_space, kg)
        ...
        return {'action_sample': (next_r, next_e), 'q_values': q_values_.squeeze(dim=-1)}
