...
class RewardShapingREINFORCE(REINFORCE):
    def __init__(self, args, kg, pn, fn_kg, fn, fn_secondary_kg=None):
        super(RewardShapingREINFORCE, self).__init__(args, kg, pn)
        ...
        if fn_model == 'hypere':
            ...
            utils.detach_module(self.fn_secondary_kg)
    ...
    def reward_fun(self, e1, r, e2, pred_e2):
        if self.model.endswith('.rso'):
            ...
            if self.model.endswith('rsc'):
                return real_reward, 0.
            else:
                binary_reward = (pred_e2 == e2).float()
                return binary_reward + self.mu * (1.0 - binary_reward) * real_reward + 0., binary_reward
    ...
    @property
    def fn_model(self):
        return self.model.split('.')[3]


def forward_fact_oracle(e1, r, e2, kg):
    oracle = utils.zeros_var_cuda([len(e1), kg.num_entities]).cuda()
    ...
    oracle_e2 = utils.batch_lookup(oracle, e2.unsqueeze(1))
    return oracle_e2
