...
class GraphSearchPolicy(nn.Module):
    def __init__(self, args):
        ...
        self.optimizer = torch.optim.Adam(
            filter(lambda p: p.requires_grad, self.parameters()), lr=args.actor_learning_rate)
    ...
    def transit(self, e, obs, kg, use_action_space_bucketing=True, merge_aspace_batching_outcome=False):
        ...
        def policy_nn_fun(X2, action_space):
            ...
            action_dist = F.softmax(
                torch.squeeze(A @ torch.unsqueeze(X2, 2), 2) - (1 - action_mask) * utils.HUGE_INT,
                dim=-1)  # @ 是 matmul 的意思
    ...
    def initialize_path(self, init_action, kg):
        ...
        if self.relation_only_in_path:  # 采样的路径只包含关系，不包含实体，也就是关系路径
            init_action_embedding = kg.get_relation_embeddings(init_action[0])
        else:  # 采样的路径记录下经过的实体和关系
        ...
        init_h = utils.zeros_var_cuda([self.history_num_layers, len(init_action_embedding), self.history_dim],
                                device=self.device)
        init_c = utils.zeros_var_cuda([self.history_num_layers, len(init_action_embedding), self.history_dim],
                                device=self.device)
    ...
    def get_answer_mask(self, e_space, e_s, q, kg):
        ...
        for i in range(len(e_space)):
            ...
            if not _e_s in answer_vectors or not _q in answer_vectors[_e_s]:
                answer_vector = utils.var_cuda(torch.LongTensor([[kg.num_entities]]), device=self.device)
            else:
    ...
    def get_false_negative_mask(self, e_space, e_s, q, e_t, kg):  # 应该是为了考虑除了训练样例 (e_s,q,e_t) 中的 e_t，其它正确的尾实体都 mask 住
    ...
    def define_modules(self):
        if self.relation_only:  # 搜索时动作空间只看关系
    ...
    def initialize_modules(self):
        if self.xavier_initialization:
            ...
            # nn.init.constant_(self.W1.weight, 1.414)
            nn.init.xavier_uniform_(self.W2.weight)
            # nn.init.constant_(self.W2.weight, 1.414)
