...
from src.common.utils import unique_max
...
def beam_search(pn, e_s, q, e_t, kg, num_steps, beam_size, return_path_components=False):
    ...
    def top_k_action(log_action_dist, action_space):
        ...
        # [batch_size, 1]
        action_batch_offset = utils.int_var_cuda(torch.arange(batch_size) * last_k, device=e_s.device).unsqueeze(1)
        # [batch_size, k] => [batch_size*k]
    ...
    def top_k_answer_unique(log_action_dist, action_space):
        ...
        for i in range(batch_size):
            ...
            r_space_b = r_space[i]
            e_space_b = e_space[i]
            unique_e_space_b = utils.var_cuda(torch.unique(e_space_b.data.cpu()), device=e_s.device)
        ...
        next_r = utils.pad_and_cat(next_r_list, padding_value=kg.dummy_r).view(-1)
        next_e = utils.pad_and_cat(next_e_list, padding_value=kg.dummy_e).view(-1)
    ...
    # Initialization
    r_s = utils.int_fill_var_cuda(e_s.size(), kg.dummy_start_r, device=e_s.device)
    seen_nodes = utils.int_fill_var_cuda(e_s.size(), kg.dummy_e, device=e_s.device).unsqueeze(1)
    init_action = (r_s, e_s)
    # path encoder
    ...
    # [batch_size*k], k=1
    log_action_prob = utils.zeros_var_cuda(batch_size, device=e_s.device)
    ...
    for t in range(num_steps):
        ...
        if return_path_components:
            utils.rearrange_vector_list(log_action_probs, action_offset)
    ...
    if return_path_components:
        ...
        for i in range(batch_size):
            ...
            for k, log_action_prob in enumerate(log_action_probs):
                ...
                p_c.append((e_name, top_k_edge_labels, utils.var_to_numpy(top_k_action_prob)))
